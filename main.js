...
$(function() {
  ...
  function set_settings() {
	...
	settings.condition_1_superlikes = [126, 238, 299, 302, 868, 1034, 1394]; 
	settings.condition_1_likes = [151, 276, 729]; 
    ...
    settings.condition_2_likes = [20000,60000,90000,150000];  
	...
	settings.condition_1_adjusted_likes = [12000, 15000,35000,80000]; // 4
	settings.condition_1_adjusted_superlikes = [12000, 15000,35000,80000]; // 4
	settings.condition_2_adjusted_likes = [12000, 15000,35000,80000]; // 4
    ...
    settings.superlikes_by = ['Sunsright','Mark','Jade','Taremine','Jess','Alex','Vicsvaporub']; 
	settings.likes_by = ['Jessicam','Kuroromi','DavidsonIan']; 
  }
  ...
  function init_task() {
		  ...
		  "posts" : [
			{
			  ...
			  "superlikes": window.settings.condition_superlikes,
			  "likes": window.settings.condition_likes,
			  "names": window.settings.superlikes_by,
    ...
    // When user receives superlikes
	  $('.userssuperlikes').each(function() {
  		var that = $(this);
  		var names = $(this).data('names').split(",");
  		var times = $(this).data('superlikes').split(",");

  		for(var i=0; i<times.length; i++) 
  		{ 
  			times[i] = +times[i]; 
  			
  			themsg = names[i] + " superliked your post";
    ...
    // When others receive superlikes
	  $('.otherssuperlikes').each(function() {
  		var that = $(this);
  		var times = $(this).data('superlikes').split(",");
    ...
    // When others receive likes
	  ...
	  let socialCredits = 25;
	  let superLikeCost = 5;
	  let likeCost = 1;
	  const socialCreditsEl = document.createElement("div");
	  socialCreditsEl.className ="social-credits-counter";
	  $("#countdown").prepend(socialCreditsEl);
	  function showsocialCreditsValue(){
		socialCreditsEl.innerHTML = "Social Credits left <br> <span>"+socialCredits+"</span>"
	  }
	  showsocialCreditsValue();
    // Initialize like buttons
	  $('.btn-superlike').on('click', function() {
		if(socialCredits>=superLikeCost){
			$(this).prev().text(parseInt($(this).prev().text()) + 1);

			socialCredits-=superLikeCost;
		}
		showsocialCreditsValue();
      // Like buttons can only be clicked once
		...
		if(socialCredits < superLikeCost){
			$('.btn-superlike').attr("disabled", true);
		}
	  
	  });

    // Initialize like buttons
	  $('.butt-like').on('click', function() {
		if(socialCredits>=likeCost){
			$(this).prev().text(parseInt($(this).prev().text()) + 1);
			
			socialCredits-=likeCost;
		}
		showsocialCreditsValue();
      // Like buttons can only be clicked once
		$(this).attr("disabled", true);
	  ...
	  $('.bottomhalf').append('<p class="credits-received">Social Credits received<span class="badge badgers-custom">0</span></p>');
	  function updateCreditsReceived(){
		$('.bottomhalf').each(function(i,el){
			let parentEl = $(el);
			let superLikes = Number(parentEl.find(".otherssuperlikes,.userssuperlikes").text());
			let likes = Number(parentEl.find(".otherslikes,.userslikes").text());
			let total = superLikes * superLikeCost + likes * likeCost;
			parentEl.find(".credits-received span").text(total);
		})
	  }
	  
	  $(".bottomhalf .otherssuperlikes,.bottomhalf .otherslikes,.bottomhalf .userssuperlikes,.bottomhalf .userslikes").on('DOMSubtreeModified', updateCreditsReceived);
	  updateCreditsReceived();
  ...
  function adjust_to_condition() {
	...
	switch(condition) {
		case 1:
			window.settings.condition_superlikes = settings.condition_1_superlikes;
			window.others.posts[1].superlikes = settings.condition_1_adjusted_superlikes;
